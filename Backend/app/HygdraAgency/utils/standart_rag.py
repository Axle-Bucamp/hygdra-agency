...
import HygdraAgency.utils.embedding
...
async def retrieve(context: RunContext[Deps], search_query: str) -> str:
    ...
    """
    # generalise model in ctx ?
    embedding = await embedding.embedding(
            content=search_query
    )

    embedding_json = pydantic_core.to_json(embedding).decode()
    rows = await context.deps.pool.fetch(
        f"""SELECT path, title, content FROM {context.deps.project.id} ORDER BY embedding <-> $1 LIMIT 8""",
        embedding_json,
    )
    return '\n\n'.join(
